  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-1 text-sm text-slate-400">
        ...
        <Link href="/minutes" className="text-sunset-300 hover:underline">
          minutes
        </Link>{" "}
        page.
      </p>
        ...
        {meetings.length === 0 ? (
          ...
          meetings.map((m) => (
            <Link
              ...
              className="flex items-center justify-between rounded-xl border border-white/10 bg-dusk-800/40 px-4 py-3 hover:border-sunset-500/40"
            >
              <span>
                <span className="font-semibold">{m.title}</span>
                <span className="ml-2 text-sm text-slate-500">{m.date ?? "no date"}</span>
              </span>
              <span className="text-xs text-slate-400">{m.present} present</span>
            </Link>
          ))
        )}
      </div>
