    ...
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
      ...
      <form action={saveRosterAction} className="mt-6">
        <div className="grid gap-5 sm:grid-cols-2">
          {roster.teams.map((t) => (
            <div key={t.id} className="rounded-2xl border border-white/10 bg-dusk-800/40 p-4">
              <input
                name={`teamname_${t.id}`}
                defaultValue={t.name}
              ...
              <div className="space-y-2">
                {t.players.map((p) => (
                  <div key={p.id} className="flex items-center gap-2">
                    <span className="w-5 font-mono text-xs text-slate-500">{p.slot}</span>
                    <input
                      name={`pname_${p.id}`}
                      defaultValue={p.name}
                      placeholder="Name"
        ...
        <div className="sticky bottom-4 mt-6 flex items-center justify-end rounded-xl border border-white/10 bg-dusk-900/90 px-4 py-3 backdrop-blur">
          <button className="rounded-lg bg-sunset-500 px-5 py-2.5 font-semibold text-white transition hover:bg-sunset-600">
            Save roster
          </button>
        </div>
      </form>
      ...
      <div className="mt-10">
        ...
        <div className="mt-3 space-y-2">
          {roster.subs.map((s) => (
            <div
              key={s.id}
              className="flex flex-wrap items-center gap-2 rounded-xl border border-white/10 bg-dusk-800/40 p-2"
            >
              <form action={updateSubAction} className="flex flex-1 items-center gap-2">
                ...
                <input
                  ...
                  className={`${input} w-32`}
                />
                <button className="rounded-lg border border-white/10 px-3 py-2 text-sm text-slate-300 hover:bg-white/5">
                  Save
                </button>
              </form>
