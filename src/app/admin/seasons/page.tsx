...
export default async function SeasonsArchive() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      {seasons.length === 0 ? (
        <div className="mt-6 rounded-2xl border border-line bg-surface/40 p-8 text-center text-ink2">
        ...
        <div className="mt-6 space-y-4">
          {seasons.map((s) => (
            <div key={s.id} className="rounded-2xl border border-line bg-surface/40 p-5">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h2 className="text-lg font-bold">{s.label}</h2>
                <span className="text-xs text-ink3">Archived {s.archivedOn}</span>
              </div>
              {s.champion && (
                <p className="mt-1 text-sm text-accent">🏆 Champion: {s.champion}</p>
              )}
              {s.standings.length > 0 && (
                <ol className="mt-3 grid gap-1 sm:grid-cols-2">
                  {s.standings.map((t) => (
                    <li
                      key={t.id}
                      className="flex items-center justify-between rounded-lg bg-surface2 px-3 py-1.5 text-sm"
                    >
                      <span>
